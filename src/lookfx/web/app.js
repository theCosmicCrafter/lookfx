...
function setTheme(name) {
  ...
  try { localStorage.setItem("lookfx.theme", name); } catch { /* */ }
  timeline.draw();
...
function setAccent(name) {
  ...
  try { localStorage.setItem("lookfx.accent", name); } catch { /* */ }
  timeline.draw();
}
...
function setTool(mode) {
  ...
  if (w) { w.value = mode; node.refresh(); } else { step.params.position_mode = mode; store.touch(i, "position_mode"); }
  updateTools();
}
function updateTools() {
  const step = store.step();
  const mode = step?.effect === "flare" ? (step.params.position_mode || "manual") : null;
...
async function openMedia(path) {
  ...
  try {
    ...
    await refreshMedia();
    remember(path, "media");
    if (!store.doc.chain.length) store.addStep("flare", { preset: { preset_file: "cine_blue" } });
    showScreen("edit");
    status(`opened ${path}`);
    setDirty(false);
...
async function refreshMedia() {
  ...
  timeline.inPoint = store.doc.input.range?.[0] || 0;
  timeline.outPoint = store.doc.input.range?.[1] != null ? store.doc.input.range[1] - 1 : null;
  timeline.draw();
  preview.invalidate();
  preview.schedule();
}
...
async function saveProject() {
  ...
  if (!path) {
    const base = (store.media?.path || "project").replace(/\.[^.]+$/, "");
...
function remember(path, kind) {
  try {
    const list = JSON.parse(localStorage.getItem("lookfx.recent") || "[]").filter((r) => r.path !== path);
    list.unshift({ path, kind, when: Date.now() });
    localStorage.setItem("lookfx.recent", JSON.stringify(list.slice(0, 8)));
  } catch { /* */ }
  renderRecent();
}
function renderRecent() {
  ...
  let list = [];
  try { list = JSON.parse(localStorage.getItem("lookfx.recent") || "[]"); } catch { /* */ }
  ...
  for (const r of list) {
    ...
    row.innerHTML = `<span class="name">${r.path.split(/[\\/]/).pop()}</span><span class="path">${r.path}</span><span class="mono dim">${r.kind}</span>`;
...
function renderLayers() {
  ...
  chain.map((s, i) => [s, i]).reverse().forEach(([s, i]) => {
    ...
    const sub = s.effect === "flare"
      ? (s.params.preset?.preset_file || s.params.preset?.name || (s.params.preset?.groups ? `${s.params.preset.groups.length} groups` : "custom")) + ` · ${s.params.position_mode || "manual"}`
    ...
    row.innerHTML = `<input type="checkbox" ${s.enabled ? "checked" : ""} title="enable">
      <span class="tint" style="background:${EFFECT_TINT[s.effect]}"></span>
      <div class="text"><span class="title">${EFFECT_LABEL[s.effect] || s.effect}</span><span class="sub">${sub}</span></div>
...
// full object, so references are expanded in place (not an undo step).
...
async function resolvePresets() {
  for (const step of store.doc.chain) {
    ...
    step.params.preset = JSON.parse(JSON.stringify(presetCache.get(ref)));
  }
}
...
async function ensureNodes() {
  ...
  for (let i = 0; i < store.doc.chain.length; i++) {
    ...
    const node = await FakeNode.create(step.effect, i, schema, { panel, viewport });
...
function layoutPicker() {
  ...
  stage.style.width = `${Math.max(sw, w)}px`; stage.style.height = `${Math.max(sh, h)}px`;
  const left = (Math.max(sw, w) - w) / 2, top = (Math.max(sh, h) - h) / 2;
  for (const el of [img, $("#view-plate")]) { el.style.width = `${w}px`; el.style.height = `${h}px`; el.style.left = `${left}px`; el.style.top = `${top}px`; }
  slot.style.cssText = `position:absolute;z-index:2;left:${left}px;top:${top}px;width:${w}px;height:${h}px;`;
  updateWipe();
  for (const n of state.nodes.values()) n._fcPicker?.draw?.();
}
function updateWipe() {
...
async function openRenderDialog() {
  ...
  const sel = $("#rd-codec"); sel.innerHTML = codecs.map((c) => `<option value="${c}">${c}</option>`).join("");
  sel.value = store.doc.output?.codec || (store.media.kind === "still" ? "still" : "prores");
  if (store.media.kind === "still" && !codecs.includes("still")) sel.innerHTML += `<option value="still">still</option>`;
  const base = (store.media.path || "output").replace(/\.[^.]+$/, "");
  $("#rd-path").value = store.doc.output?.path || `${base}_fx${store.media.kind === "still" ? ".png" : ".mov"}`;
...
async function startRender() {
  ...
  const stem = path.replace(/\.[^.]+$/, ""), ext = codec.endsWith("_seq") ? "" : (path.match(/\.[^.]+$/)?.[0] || ".mov");
  if ($("#rd-pass").checked) out.aux.flare_pass = `${stem}_pass${ext}`;
  if ($("#rd-plates").checked) out.aux.plates = `${stem}_plates${ext}`;
  const range = [+$("#rd-start").value || 0, +$("#rd-end").value || null];
...
async function refreshJobs() {
  ...
  for (const j of jobs) {
    ...
    card.innerHTML = `<div class="head"><span class="name">${j.kind}</span><span class="dim">${out || j.stage || ""}</span><div class="grow"></div><span class="state">${j.state}${j.error ? " · " + j.error : ""}</span></div>
      <div class="bar"><div style="width:${pct}%"></div></div>
      <div class="meta"><span>${j.stage || ""} ${j.done} / ${j.total}</span><span>${speed}</span><span>${eta}</span><div class="grow"></div>
        ${j.state === "running" || j.state === "queued" ? `<button class="mini" data-cancel="${j.id}">Cancel</button>` : ""}
        ${j.state === "done" && out ? `<button class="mini" data-open="${out}">Open folder</button>` : ""}</div>`;
    card.querySelector("[data-cancel]")?.addEventListener("click", () => post(`/api/jobs/${j.id}/cancel`, {}).then(refreshJobs));
...
async function renderSettings() {
  ...
  const groups = [
    ...
    ["Tools", [
      ["ffmpeg", "Decodes and encodes every clip; 16-bit RGB pipes", s.ffmpeg || "not found"],
  ...
  $("#settings-groups").innerHTML = groups.map(([g, rows]) => `<div class="sgroup"><span class="label">${g}</span><div class="box">${
    rows.map(([l, h, v]) => `<div class="srow"><div class="grow"><div>${l}</div>${h ? `<div class="hint">${h}</div>` : ""}</div><div class="val" title="${v}">${v}</div></div>`).join("")}</div></div>`).join("");
  $("#gpu").textContent = s.gpu ? `${s.gpu.name.replace("NVIDIA GeForce ", "")} · ${s.gpu.free_gb}/${s.gpu.total_gb} GB` : s.device;
  $("#status-versions").textContent = `lookfx ${s.version} · flarecore ${s.engines.flarecore} · cmyk-magic ${s.engines.cmykmagic}`;
  $("#rail-versions").innerHTML = `flarecore<br>cmyk ${s.engines.cmykmagic}`;
  $("#welcome-version").textContent = s.version;
}

// ---------------------------------------------------------------- store wiring
store.subscribe(async (what, detail) => {
  if (what === "load") { renderLayers(); await ensureNodes(); updateLanes(); preview.invalidate(); preview.schedule(); }
  if (what === "stack") { renderLayers(); await ensureNodes(); updateLanes(); setDirty(true); preview.schedule(); pushProject(); }
  if (what === "select") { renderLayers(); showSelectedNode(); }
  if (what === "params") { renderLayers(); updateTools(); setDirty(true); preview.schedule(); }
  if (what === "output") { setDirty(true); }
  if (what === "frame") {
...
// ---------------------------------------------------------------- boot
async function boot() {
  try { setTheme(localStorage.getItem("lookfx.theme") || "dense"); setAccent(localStorage.getItem("lookfx.accent") || "amber"); }
  catch { setTheme("dense"); setAccent("amber"); }
  const effects = await api("/api/effects");
  for (const e of effects) state.schemas[e.id] = await api(`/api/effects/${e.id}/schema`);
  ...
  $("#btn-add-flare").onclick = () => store.addStep("flare", { preset: { preset_file: "cine_blue" } });
  $("#btn-add-print").onclick = () => store.addStep("print_look", {});
  $("#btn-depth").onclick = async () => {
    const p = await host.pickFile("Depth map (clip or image)");
    if (!p) return;
    store.mutate((d) => { d.aux = { ...d.aux, depth: { path: p } }; }, "stack");
  ...
  $("#btn-depth-clear").onclick = async () => {
    store.mutate((d) => { delete d.aux.depth; }, "stack");
    await put(`/api/project/${store.projectId}`, store.doc);
    await refreshMedia();
  };
  ...
  $("#tp-in").onclick = () => { timeline.inPoint = store.frame; timeline.draw(); };
  $("#tp-out").onclick = () => { timeline.outPoint = store.frame; timeline.draw(); };
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (e.key.toLowerCase() === "i") { timeline.inPoint = store.frame; timeline.draw(); }
    if (e.key.toLowerCase() === "o") { timeline.outPoint = store.frame; timeline.draw(); }
  });
  window.addEventListener("beforeunload", (e) => { if (state.dirty) { e.preventDefault(); e.returnValue = ""; } });

  window.lookfx = { store, openMedia, openProject, preview, timeline, state };   // debugging / launcher hooks
